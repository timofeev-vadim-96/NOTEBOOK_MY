...
> OTUS [шпаргалка по git](https://github.com/cyberspacedk/Git-commands)
...
* git commit -m "comment" - сохраняет изменения файла (перед этой командой необходимо добавть файл с помощью "add")  
  ...
  * git commit --amend -m "another comment" - изменить (переименовать) содержание текста последнего коммита
...
* git log - журнал всех изменений (коммитов)  
  ...
  * git log --grep fix - показать логи с фильтром "fix" в тексте
...
* git rm -r --cached file - удалить файл из отслеживания гитом (предполагаю, что предварительно записав в гитигнор) //-r для папок (рекурсивно)
  * git rm file.name - удалить файл физически и удалить из отслеживания
