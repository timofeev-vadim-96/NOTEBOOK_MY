...
**Инкапсуляция** – это свойство системы, позволяющее объединить данные и методы, работающие с ними в классе, скрыв детали реализации и защитив от пользователя этого класса объектов.  # *это значит описать поля класса, его конструкторы и методы, скрыв при этом от пользователя возможность вызывать и изменять то, что не следует* *и что не мало важно - не дать пользователю сломать логику выполнения программы, если компьютеру чтобы работать нужно включиться, то не дать пользователю попытку заставить компьютер работать до того, как он включился*
  * Инкапсуляция предназначена для изоляции контрактных обязательств (в интерфейсе) от их реализации (в объекте, который совершает имплементацию). На практике это означает, что класс должен состоять из двух частей: интерфейса (не тип класса, а абстрактное понятие) и реализации.
...
**Полиморфизм** – возможность объектов с однинаковой спецификацией иметь различную реализацию (переписывать @Override) методы. + возможность перегружать методы (разное количество и тип входные аргументов).  далее не оч понятно - это свойство системы, использовать объекты с одинаковым интерфейсом без информации о типе и внутренней структуре объекта. Полиморфизм - способность использовать объект вне зависимости от его реализации, благодаря, полиморфной переменной – это переменная, которая может принимать значения разных типов.
...
**Хэш код** - это адрес на ячейку в памяти, где хранится то или иное значение.  
...
Библиотека pandas может читать многие форматы, включая: .csv, .xslx, .xls, .txt, sql и многие другие. Полный список по [ссылке](https://pandas.pydata.org/pandas-docs/stable/_guide/io.html "pandas-docs")  
...
`Стэк` (как стопка книг, доступ только к верхней книге) - для примитивных переменных и идентификаторов //LIFO - принцип
  * каждый поток имеет свой Стэк
Особенности Стэка:
1. Заполняется и освобождается по мере вызова и завершения новых методов;
2. Переменные на стеке существуют до тех пор, пока выполняется метод в котором они были созданы;
...
5. Является потокобезопасным, поскольку для каждого потока создаётся свой отдельный стек.  
`Heap` (управляемая куча) - для переменных с неопрд. размером (все объекты, строки и т.д.)  
При этом: идентификаторы хранятся всегда в стеке, а значение может хранится уже и в хипе. Так с массивами. 
  Поколения кучи:
  * young generation - область, где размещаются недавно созданные объекты (когда заполняется, происходит быстрая сборка мусора)
  * old generation - хранятся долгоживущие объекты
  * permanent generation (до java 8.0), metaspace (java > 8.0) - хранятся метаданные, статические поля
Особенности кучи:
1. В общем случае, размеры кучи на порядок больше размеров стека;
2. Когда эта область памяти полностью заполняется, Java бросает
3. java.lang.OutOfMemoryError;
4. Доступ к ней медленнее, чем к стеку;
5. Эта память, в отличие от стека, автоматически не освобождается. Для сбора неиспользуемых объектов используется сборщик мусора;
6. В отличие от стека, который создаётся для каждого потока свой, куча не является потокобезопасной, поскольку для всех одна, и ее необходимо контролировать, правильно синхронизируя код
** строки (String) в куче хранятся в специальном отделе String pool, который хранит уникальные значения строк. (если существуют две строки с одинаковыми значениями, то в бассейн попадает только первое). Добавление в бассейн называется - интернированием.

ХЗ где, `доп`. зоны памяти:
`Method Area/Metha space` – это область памяти, где хранятся информация о классах и методах JVM.
Здесь также хранятся константы и статические переменные.
`Program Counter Register` – это регистр, который указывает на следующую инструкцию,
которую нужно выполнить в текущем потоке.
`Native Method Stack` – это стек, используемый для выполнения нативного кода.

Ключевое слово `volatile` указывает, что взаимодействие с переменной в памяти должно происходить минуя кэши процессора, т. е. напрямую. //слово volatile запрещает делать копию на кэш ядра процессора
...
* Последовательный; //замораживает все потоки при запуске (не рекомендуется в многопоточных приложениях)
* Параллельный - по умолчанию. //использует несколько потоков для сборки мусора. Также замораживает все остальные
* CMS; //несколько потоков, более короткие паузы при сборке мусора. Приложения не перестают рабоатть при сборке мусора, но, в среднем, работают медленнее из-за параллельной сборки мусора
* G1; //для приложений, работающих на многопроцессорных компьютерах с большим объемом памяти. (более эффективный)
* ZGC //масштабируемый сборщик с низкой задержкой (с java 15). Не останавливает потоки приложения более, чем на 10мс
...
**Двусвязный список (LinkedList)** - каждый элемент хранит теперь в себе ссылку не только на след. элемент, но  
...
(`View` - все, что связано с представлением данных, интерфейс. Рожает html-страницу 
`Controller` - логика приложения   (Http Request)
`Model` - хранит модели данных (классы, объекты)), взаимодействие с БД
...
классов  (есть некий класс Creator, который в зависимости от условий (if) возвращает объект какого-то типа. А типы, в общем случае, имплементируются или наследуют от какого-то общего)  
  * либо static метод внутри создаваемого класса
  типа:
      public static RockMusic fabric(){ return new RockMusic();}
...
Идея: есть интерфейс с одним методом, который возвращает поля класса (Characteristic). В этом классе перечислены все возможные параметры товаров. Но у каждого товара может быть всего несколько параметров из общего перечня. Для этого:
...
6. Паттерн Singleton: для того, чтобы объект класса создавался только один раз
---
...
* `Liskov substitution principle` (постановки Барбары Лисков)  
...
Программа для **времени выполнения** задач LeetCode: https://rextester.com/l/php


> Добавленин Hyper-V для Домашней Windows (для работы с Docker/докером) + нужно подрубить WSL2: https://lumpics.ru/no-hyper-v-component-in-windows-10/


> Чтобы запустить java-приложение из .exe
0. использовать программу launch4j. На первой вкладке указать название исполняемого файла и путь к JAR
1. jlink --module-path $JAVA_HOME/jmods --add-modules java.base,java.desktop --output customjre
В этом примере:
- jlink - вызывает инструмент jlink.
- --module-path $JAVA_HOME/jmods - указывает путь к модулям JDK. $JAVA_HOME представляет переменную среды, которая указывает на корневую папку установленной JDK.
- --add-modules java.base,java.desktop - указывает модули, которые должны быть включены в JRE. Здесь указываются модули, необходимые для базового функционирования и поддержки графического интерфейса приложения.
- --output customjre - указывает имя папки, в которую будет сохранен созданный образ JRE.
2. на вкладке JRE указать название папки customjre
3. собрать конфиг нажав на шестеренку
...
5. Решение задач: https://www.hackerrank.com/  //все хуйня - leetcode
6. Решение задач: https://www.codewars.com/  //все хуйня - leetcode
...
12. Свой PPA - репозиторий: https://launchpad.net/ просто зарегистрироваться (для контейнеров ???)
13. https://app.diagrams.net/ - блок-схемы, диаграммы
