...
```yml
  h2:
    console:
      enabled: true
```
  * для просмотра базы данных, если это необходимо, открыть URL http://localhost:8080/h2-console
...
Пример `OncePerRequestFilter`. В данном примере проверяется наличие хедера в запросе.
```java
public class MdcFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (xRequestId != null) {
            //добавляем id запроса для логгера, которое будет отображаться во всех дальнейших действия относительно данного запроса в Логах данной ThreadLocal
        ...
        //после обработки запроса убираем requestId из параметров для Логгеров.
...
`Pageable`
**Pageable в методах контроллеров**
```java
...
`Создать экземпляр PageImpl`
```java
PageImpl<BookDto> books = new PageImpl<>(expected, pageable, expected.size());
```

Параметры запроса для клиента:
```bash
GET /my-entities?page=0&size=10&sort=name,asc
```